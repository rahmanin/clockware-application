...
adminRouter.post(
  ...
  (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      const selectLastAdded =
        "SELECT * FROM cities WHERE id=(SELECT MAX(id) FROM cities)";
...
adminRouter.put(
  ...
  (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      const sql =
        "UPDATE orders SET feedback_master=$1, additional_price=$2, is_done=$3 WHERE order_id=$4 AND master_id=$5";
      const orderSql =
        "SELECT size, city, order_date, order_time_start, order_master, feedback_master, order_price, additional_price FROM orders WHERE order_id=$1";
      ...
      db.any(sql, toFinishOrder)
        ...
        .then(
          db.any(orderSql, [order_id]).then((result) => {
...
adminRouter.post("/api/check_token", getAccess, (req, res) => {
  const { userId, is_admin } = req.userData;
